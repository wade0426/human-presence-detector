"""需求三：音檔試聽按鈕（FR-3.1～3.7）、FR-2.7 瀏覽過濾、§4.7 settings 以磁碟為基底。"""
...
from src.ui.settings import SOUND_FIELD_KEYS, SettingsWindow, SoundPathField
...
def test_preview_button_has_speaker_icon(qtbot: pytest.QtBot, tmp_path: Path) -> None:
    """FR-3.1：試聽按鈕需帶喇叭圖示（非純文字按鈕），播放中換成停止圖示。"""
    window = _make_window(qtbot, tmp_path)
...
def test_undecodable_file_shows_error_hint_via_real_player(
    ...
    assert hint.property("role") == "danger"
    assert field.preview_button.text() == strings.SOUND_PREVIEW_PLAY
    assert window._preview_key is None


@pytest.mark.qt
def test_preview_natural_finish_restores_button(
    qtbot: pytest.QtBot, tmp_path: Path
) -> None:
    """單次播放自然結束（StoppedState）→ 按鈕復原、_preview_key 清除。"""
    fake = FakePlayer()
    window = _make_window(qtbot, tmp_path, fake)
    sound = _sound_file(tmp_path)
    ...
    window._on_preview_playback_state(QMediaPlayer.PlaybackState.StoppedState)
...
def test_preview_error_callback_restores_button_and_sets_hint(
    ...
    window._on_preview_error(QMediaPlayer.Error.FormatError, "boom")
...
def test_sound_field_browse_uses_audio_filter(qtbot: pytest.QtBot, tmp_path: Path) -> None:
    ...
    for key in SOUND_FIELD_KEYS:
        ...
        assert isinstance(field, SoundPathField)
        with patch(
            "src.ui.settings.QFileDialog.getOpenFileName", return_value=("", "")
        ) as mock_open:
            field._browse()
...
@pytest.mark.qt
def test_save_preserves_disk_roi_over_stale_config(
    qtbot: pytest.QtBot, tmp_path: Path
) -> None:
