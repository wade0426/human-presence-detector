...
from src.infra.config import FloatingConfig, ReminderConfig
...
def test_load_pixmap_missing_path_returns_placeholder() -> None:
    from src.shell.reminders.media import load_pixmap
...
# ---------------------------------------------------------------------------
# §4.14 work_minutes float：FloatingReminder 倒數秒數（自舊 tests/test_reminder.py 移植）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PopupReminder single button, ReturnPromptDialog（自舊 tests/test_reminder.py 移植）
# ---------------------------------------------------------------------------
...
def test_popup_reminder_has_single_action_button(qtbot: pytest.QtBot) -> None:
    ...
    from src.shell.reminders.popup import PopupReminder
    from src.shell.strings import REMIND_START_REST
...
def test_return_prompt_dialog_show_prompt_keeps_plain_body_text(qtbot: pytest.QtBot) -> None:
    from src.shell.reminders.return_prompt import ReturnPromptDialog
    from src.shell.strings import RETURN_BODY
...
# ---------------------------------------------------------------------------
# FR-2.2/2.4/2.5 — PopupReminder 提醒音獨立 QMediaPlayer、hide 停音（移植）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 重設計新增（spec §9）：presence 模式下 popup 按鈕文案「開始休息（請離席）」
# ---------------------------------------------------------------------------


@pytest.mark.qt
def test_remind_start_rest_presence_string_value() -> None:
    from src.shell.strings import REMIND_START_REST_PRESENCE

    assert REMIND_START_REST_PRESENCE == "開始休息（請離席）"


@pytest.mark.qt
def test_popup_reminder_presence_mode_button_text(qtbot: pytest.QtBot) -> None:
    """presence 模式建構 → 按鈕文案為「開始休息（請離席）」，show() 後仍維持。"""
    from src.shell.reminders.popup import PopupReminder
    from src.shell.strings import REMIND_START_REST_PRESENCE

    reminder = PopupReminder(presence_mode=True)
    qtbot.addWidget(reminder)

    assert reminder.dismiss_button.text() == REMIND_START_REST_PRESENCE
...
@pytest.mark.qt
def test_factory_passes_presence_mode_to_popup(qtbot: pytest.QtBot) -> None:
    """factory rest_count_mode='presence' → popup 按鈕為離席文案；預設維持舊文案。"""
    from src.shell.reminders.factory import create_reminder
    from src.shell.reminders.popup import PopupReminder
    from src.shell.strings import REMIND_START_REST, REMIND_START_REST_PRESENCE

    presence = create_reminder(ReminderConfig(method="popup"), rest_count_mode="presence")
    qtbot.addWidget(presence)
    assert isinstance(presence, PopupReminder)
    assert presence.dismiss_button.text() == REMIND_START_REST_PRESENCE

    fixed = create_reminder(ReminderConfig(method="popup"), rest_count_mode="fixed")
    qtbot.addWidget(fixed)
    assert fixed.dismiss_button.text() == REMIND_START_REST
